Shader "Move/PBR_Eye_Shader_Custom"
{
	Properties
	{
		_scleraColor ("_scleraColor", Color) = (0.95,0.95,0.95,1)
		_irisColor ("_irisColor", Color) = (1,1,1,1)
		_illumColor ("_illumColor", Color) = (0,0,0,0)

		_pupilSize("_pupilSize", Range(0.0,1.0)) = 0.27
		_irisSize("虹膜大小", Range(1.5,5.0)) = 1.88
		_parallax("_parallax", Range(0.0,0.1)) = 0.05
		_scleraSize("巩膜大小", Range(0.85,2.2)) = 0.968
		_limbus("_limbus", Range(0.0,1.0)) = 0.5
		_specsize("_specsize", Range(0.0,1.0)) = 0.9
		_smoothness("_smoothness", Range(0.0,1.0)) = 0.75
		_reflectTerm("_reflectTerm", Range(0.0,1.0)) = 0.025
		_brightShift("_brightShift", float) = 1.0
		
		_scleraShadowAmt("巩膜阴影遮蔽程度", Range(0.0,1.0)) = 0.0
		_irisShadowAmt("虹膜阴影遮蔽程度", Range(0.0,1.0)) = 0.0

		_IrisColorTex ("_IrisColorTex", 2D) = "white" {}
		_IrisMaskTex ("_IrisMaskTex", 2D) = "white" {}
		_CorneaBump ("_CorneaBump", 2D) = "bump" {}
		_EyeBump ("_EyeBump", 2D) = "bump" {}
		_IrisBump ("_IrisBump", 2D) = "bump" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			// sampler2D _IrisColorTex,_IrisMaskTex,_ShadeScleraTex,_ShadeIrisTex;
			// float _scleraShadowAmt,_irisShadowAmt;
			// float4 _albedoColor,reflectionMatte,irradianceTex,albedoColor;
			// float _roughness,_reflective,_metalMap,_ambientMap,_irisSize,_scleraSize,_pupilSize,_limbus;
			// sampler2D _CorneaBump,_EyeBump,_IrisBump;
			// float4 _scleraColor,_irisColor,_irisColorB,_pupilColor,_illumColor;
			// float _parallax,_brightShift,irisoffsettex,_smoothness,_specsize,_reflectTerm;


			sampler2D _IrisColorTex;
			sampler2D _IrisMaskTex;
			sampler2D _MainTex;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				// 计算法线贴图
				half3 cBump = UnpackNormal(tex2DBySize(_CorneaBump,i.uv,_irisSize));

				half iSize2 = _irisSize*0.5;
				half3 iBump = UnpackNormal(tex2DBySize(_IrisBump,i.uv,iSize2));

				// 计算巩膜贴图
				half4 scleratex = tex2DBySize(_MainTex,i.uv,_scleraSize);
				scleratex.rgb = lerp(scleratex.rgb,scleratex.rgb*_scleraColor.rgb,_scleraColor.a);

				half3 eBump = UnpackNormal(tex2DBySize(_EyeBump,i.uv,_scleraSize));

				// 计算巩膜AO
				half scleraShade = tex2DBySize(_ShadeScleraTex, i.uv,_scleraSize);
				scleratex.rgb *= 1 - (scleraShade * _scleraShadowAmt);

				// 计算虹膜Mask贴图
				irismasktex = tex2DBySize(_MainTex, i.uv,_irisSize).a;// float2((i.uv.x*_irisSize)-((_irisSize-1.0)/2.0),(i.uv.y*_irisSize)-((_irisSize-1.0)/2.0))).a;

				// 混合法线
				o.Normal = lerp(eBump,cBump,irismasktex);

				// 获取Mask贴图
				half uvMask = 1.0-tex2D(_IrisMaskTex,i.uv).b;

				// 计算虹膜纹理贴图
				half iSize = _irisSize * 0.6;
				float2 irUVc = i.uv;
				irUVc = CenterZoomUVBySize(i.uv,iSize);

				/*
					float pupilSize = lerp(lerp(0.5,0.2,iSize/5),lerp(1.2,0.75,iSize/5),_pupilSize);
					(1-0.2*iSize) * 0.5 + 0.2*iSize * 0.2 = 0.5 - 0.06 * iSize;
					(1-0.2*iSize) * 1.2 + 0.2*iSize * 0.75 = 1.2 - 0.09 * iSize;
					float pupilSize = (0.5 + 0.3 * iSize )* (1 - _pupilSize) + (1.2 - 0.09 * iSize )  * _pupilSize;
				*/
				...
				half plxtex = tex2D(_IrisMaskTex,irUVp).g;
				...
				float height = tex2D(_IrisMaskTex,irUV).b;
				...
				half cNdotV2 = saturate(lerp(-0.2,0.25,max(0,dot(eBump,tangentViewDir))));
				float2 offset2 = ParallaxOffset(height, 0.5*cNdotV2,vDir);
				float irisoffsettex = tex2D(_IrisColorTex, irUVc-offset2).a;
				...
				// 计算皮下 // 没用到
				// half lightFac = max(0,dot(tangentLightDir,o.Normal)); 
				// half h3 = max(0,dot(iBump,tangentLightDir));
				// half edge2 = saturate(lerp(0.5,-0.5,max(0,dot(o.Normal,tangentViewDir))));
			    ...
			    half3 tangentHalfDir = normalize(normalize(tangentLightDir)+tangentViewDir);
			    half NdotH = max(0,dot(useNormal,tangentHalfDir));
				...
				// 角膜缘 变暗
				o.Albedo = lerp(o.Albedo, o.Albedo * (0 - (_limbus * 20.0)), saturate(irisoffsettex * (2.0-irismasktex) * irismasktex * cNdotV2) );

				// UNITY 5 Features 计算高光
